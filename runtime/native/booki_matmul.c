...
static void matmul_f16_scalar_slice(const booki_f16* A, const booki_f16* B, booki_f16* C,
...
static void scalar_worker(int64_t begin, int64_t end, void* v) {
    scalar_ctx* c = (scalar_ctx*)v;
    matmul_f16_scalar_slice(c->A, c->B, c->C, c->M, c->N, c->K, begin, end);
}

static void matmul_f16_scalar(const booki_f16* A, const booki_f16* B, booki_f16* C,
                              int64_t M, int64_t N, int64_t K) {
    scalar_ctx ctx = { A, B, C, M, N, K };
    /* Per-row independence; grain 1 means any thread count is valid. */
    booki_parallel_for(M, 1, scalar_worker, &ctx);
}
...
/* Row-slice variant. Computes rows [m_begin, m_end) of C. The full M is
 * still needed for the M-tail path (rows past the last TM-aligned tile),
 * which only the slice that owns that tail executes. */
static void matmul_f16_neon_slice(const booki_f16* A_u, const booki_f16* B_u,
                                  booki_f16* C_u,
                                  int64_t M, int64_t N, int64_t K,
                                  int64_t m_begin, int64_t m_end) {
    ...
    /* Clip the slice to TM-aligned tiles. The M-tail (rows Mt..M-1) is
     * owned by whichever slice contains them. */
    int64_t m_lo = m_begin;
    int64_t m_hi = m_end < Mt ? m_end : Mt;

    for (int64_t m = m_lo; m < m_hi; m += TM) {
    ...
    /* M-tail (rows Mt..M-1). Only the slice that contains the tail runs it.
     * Since tile chunks are TM-aligned (the parallel_for grain is TM), the
     * slice with m_end == M (i.e. the last worker) owns this. */
    if (m_end >= M) {
        int64_t tail_lo = Mt > m_begin ? Mt : m_begin;
        for (int64_t m = tail_lo; m < M; ++m) {
            for (int64_t n = 0; n < N; ++n) {
                float acc = 0.0f;
                for (int64_t k = 0; k < K; ++k)
...
static void neon_worker(int64_t begin, int64_t end, void* v) {
    neon_ctx* c = (neon_ctx*)v;
    matmul_f16_neon_slice(c->A, c->B, c->C, c->M, c->N, c->K, begin, end);
}

static void matmul_f16_neon(const booki_f16* A, const booki_f16* B, booki_f16* C,
                            int64_t M, int64_t N, int64_t K) {
    neon_ctx ctx = { A, B, C, M, N, K };
    /* Grain = TM (4); workers always get 4-row aligned chunks so the
     * NEON tile loop doesn't have to split across threads. */
    booki_parallel_for(M, 4, neon_worker, &ctx);
}
