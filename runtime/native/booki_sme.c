...
/* Streaming kernel — runs the entire matmul inside one __arm_streaming
 * scope so SVL is consistent and ZA is owned end-to-end. Caller has
 * already packed A and B; this just tiles, FMOPAs, and stores. */
__arm_locally_streaming
__arm_new("za")
static void sme_matmul_stream(const __fp16* Apack, const __fp16* Bpack,
                              float* Cf32,
    ...
    for (int64_t i = 0; i < M; i += tile) {
        int64_t tm = (M - i) < tile ? (M - i) : tile;
        for (int64_t j = 0; j < N; j += tile) {
            ...
            for (int64_t kp = 0; kp < k_pairs; ++kp) {
                svfloat16_t a = svld1_f16(pa, &Apack[kp * 2 * M + i * 2]);
...
int booki_matmul_f16_sme(const booki_tensor* a, const booki_tensor* b,
    ...
    sme_matmul_stream(Apack, Bpack, Cf32, M, N, K_pad);
