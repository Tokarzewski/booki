...
/* 1-thread vs 2-thread run of the active backend on a non-trivial shape.
 * The threaded path is supposed to be bit-identical-up-to-fp16-precision
 * — i.e. the only float-vs-float difference between runs should be from
 * the final fp32 → fp16 narrowing, which is deterministic given the same
 * fp32 accumulator. Splitting M doesn't change the per-output-element
 * reduction order (each C[m,n] still does the full K-loop on one thread),
 * so the result should be exactly equal, not just close. */
static int threading_agrees_test(booki_arena* a) {
    const int64_t M = 256, K = 128, N = 512;  /* per task spec: 256x512x128 */
    int64_t elemsA = M * K, elemsB = K * N;

    float* abuf = (float*)malloc(sizeof(float) * elemsA);
    float* bbuf = (float*)malloc(sizeof(float) * elemsB);
    for (int64_t i = 0; i < elemsA; ++i)
        abuf[i] = (float)((i * 31 + 7) % 17) * 0.05f - 0.4f;
    for (int64_t i = 0; i < elemsB; ++i)
        bbuf[i] = (float)((i * 23 + 11) % 19) * 0.05f - 0.45f;

    booki_tensor A = mk(a, M, K); fill_from(&A, abuf, elemsA);
    booki_tensor B = mk(a, K, N); fill_from(&B, bbuf, elemsB);
    booki_tensor C1 = mk(a, M, N); memset(C1.data, 0, C1.nbytes);
    booki_tensor C2 = mk(a, M, N); memset(C2.data, 0, C2.nbytes);

    int saved = booki_get_threads();

    booki_set_threads(1);
    CHECK(booki_get_threads() == 1, "set_threads(1) didn't take");
    CHECK(booki_matmul_f16(&A, &B, &C1) == 0, "1-thread run failed");

    booki_set_threads(2);
    CHECK(booki_get_threads() == 2, "set_threads(2) didn't take");
    CHECK(booki_matmul_f16(&A, &B, &C2) == 0, "2-thread run failed");

    booki_set_threads(saved);

    fp16* p1 = (fp16*)C1.data;
    fp16* p2 = (fp16*)C2.data;
    int64_t bit_mismatches = 0;
    float worst = 0.0f;
    for (int64_t i = 0; i < M * N; ++i) {
        if (p1[i] != p2[i]) bit_mismatches++;
        float d = fabsf(f16_to_f32_(p1[i]) - f16_to_f32_(p2[i]));
        if (d > worst) worst = d;
    }
    /* Strict: per-output reduction order is identical between the 1- and
     * 2-thread runs, so the result should be bit-equal. */
    CHECK(bit_mismatches == 0,
          "1-thread vs 2-thread disagree at %lld of %lld outputs (worst |Δ|=%.6f)",
          (long long)bit_mismatches, (long long)(M * N), worst);
...
#endif
    rc |= threading_agrees_test(a); booki_arena_reset(a);
